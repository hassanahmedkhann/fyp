...
const ProductBox = ({ productData, ...props }) => {
  ...
  return (
    <div className="productBox my-3 mx-3">
      ...
      {props.rating ? (
        <h5 className="mt-2">The product has the highest rating!</h5>
      ) : (
      ...
      <div className="d-flex">
        <div className="producBox-detailBox d-flex flex-column mx-1 align-items-center w-100">
          <h5 className="my-2">{(productData?.title)?.toString().slice(0,10)}</h5>
          <h5 className="my-1">{productData?.category}</h5>
          <h5 className="my-1">
            Rating:{" "}
            <Rating
              name="read-only"
        ...
        <div className="producBox-detailBox d-flex flex-column mx-1 align-items-center w-100">
          <h5 className="my-1">Price: {productData?.price}</h5>
          <h5 className="my-1">ID: {productData?.id}</h5>
          <h5 className="my-1">
            Total Profit:{" "}
            {numeral(productData?.price * productData?.price)
              .format("(0.00 a)")
              .toUpperCase()}
          </h5>
        </div>
      ...
      <h3>
        {numeral(productData?.price).format("(0.00 a)").toUpperCase()} total
        sales
      </h3>
      <Button style={linkStyle} onClick={handleModal}>
        Detailed Analysis
      </Button>
      <ProductAnalysis
