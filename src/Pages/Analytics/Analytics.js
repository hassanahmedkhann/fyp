...
import {
  getOverallAverage,
  getOverallProductAnalytics,
  getOverallProductGrowth,
  getRequest,
  postRequest,
} from "../../Api-Interaction/api-Interaction";
import { menuItemStyle, selectStyle } from "../../Util";
import { graphsCount, MenuItems, menuItems, smallGraphs, staticGraphData } from "../../Utils/constants";
const Analytics = () => {
  ...
  return (
    <div className="analyticsMain fadeUp pt-4">
      ...
      <div className="analytics-heading">
        <h1 className="text-center p-3">Overall Analytics</h1>
      </div>
      {/* Overall 1 */}
      <div className="analytics">
        <div className="analytics-graph1 d-flex flex-column justify-content-space px-2 pt-4">
          ...
          <strong style={{ color: "#F3623A" }} className="my-2 ms-4">
            Showing data from last predicted clusters!
          </strong>
          <LineGraph graphData={clusterGraph} />
          <div
            style={{ borderTop: "1px solid lightgray" }}
            className="d-flex justify-content-between pt-2 mt-2 w-100"
          >
        ...
        <div className="analytics-items-container container-fluid">
          <div
            ...
            className="analytics-parent row row-cols-1 row-cols-md-2 row-cols-lg-3"
          >
            {graphsCount?.map((graph) => (
              <div key={graph} className="analytics-small-items  col px-3 py-4">
                <SmallLineGraph graph={graph} graphData={staticGraphData} />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="analytics">
        <div className="analytics-graph1 d-flex flex-column justify-content-space px-2 pt-4">
          ...
          <BarGraph graphData={staticGraphData} />
        </div>
        <div className="analytics-items-container container-fluid mt-0">
          <div
            style={{ borderRadius: "20px" }}
            className="parent row pt-0 row-cols-1 row-cols-md-2 row-cols-lg-3"
          >
            {graphsCount?.slice(0,3).map((graph) => (
              <div key={graph} className="analytics-small-items  col px-3 py-4">
                <SmallLineGraph graph={graph} graphData={staticGraphData} />
              </div>
            ))}
