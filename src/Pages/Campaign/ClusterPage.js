...
const ClusterPage = () => {
  ...
  const [originalAmount, setOriginalAmount] = useState(9887);
    ...
    try {
      ...
      if (resultHandle?.success === true) {
        ...
        complete.forEach((item) => {
          originalAmount =
            originalAmount + item.productInfo.unitPrice * item.quantity;
        });
  ...
  const handleDraftChange = () => {
    setEmailDraft({
      subject: draftChange.subject || emailDraft.subject,
      body: draftChange.body || emailDraft.body,
    });
  ...
  const handleEmailAPI = async () => {
    ...
    packageInfo =
      packageInfo +
      packageData.map(
        (item) => " " + item?.productInfo.productName + " x " + item.quantity
      );
  ...
  const handlePackageClose = () => {
    if (discount.length <= 0)
      return setAlert({
        flag: true,
        status: 2,
        message: "Discount field can not be left empty!",
      });
    if (discount < 0)
      return setAlert({
        flag: true,
        status: 2,
        message: "Discount field can not be negative!",
      });
    if (discount > 75)
      return setAlert({
        flag: true,
        status: 2,
        message: "Discount value not allowed!",
      });
    ...
    <div className="cluster-page">
      ...
      <div className="m-4 cluster-page-card">
        <div className="clusterPageCard">
          <div className="cluster-page-header my-3">
            <Button
              ...
              className="account-button mb-3"
              sx={BackButton}
            >
              <ArrowBackIosIcon /> Back
            </Button>
            <h1>Cluster No. {clusterData?.clusterNumber || 1}</h1>
            <h3>Total Members: {clusterData?.totalCustomers || 456}</h3>
            <button
            ...
            >
              View Package <VisibilityIcon />
            </button>
            <div className="email-container mt-3">
              <div className="subject-container">
                <label>Subject:</label>
                <p className="ms-3">{emailDraft.subject}</p>
              </div>
              <div className="body-container">
                ...
                {showPackage && (
                  <>
                    ...
                    <div className="my-2 fadeIn">
                      {clusters?.map((item = null, index) => (
                        <p key={index} className="m-0">
                          {`Product ${index + 1}` ||
                            item?.productInfo.productName}{" "}
                          x {Math.round(index * Math.random() + 1)}
                        </p>
                      ))}
                      <p className="m-0">Original cost: {originalAmount} $</p>
                      <p className="m-0">Discount %: {discount} %</p>
                      <p>
                        New cost:{" "}
                        <strong>
                          {(
                ...
                <p>
                  We hope you'd want to shop again and redeem your package!
                  <br />
                  Cheers!
                </p>
              </div>
          ...
          <div className="cluster-customer-list">
            ...
            <div className="customer-list">
              {clusters?.map((customer, index) => (
                <>
                  <h6 key={index} className="my-3 mx-1 p-3 customer-name">
                    <h6>
                      {`Name: Customer ${index + 1}` || customer?.fullName}
                    </h6>
                    <h6>{`Email: customer${index + 1}@origin8.com`}</h6>
                  </h6>
                </>
              ))}
            </div>
        ...
        <div className="cluster-page-buttons">
          <button disabled onClick={handleEmailAPI} className="btn-2 mb-3 mx-2">
            Start Email Campaign
          </button>
          <button
            onClick={() => setOpenModal(true)}
            className="btn-1 mb-3 mx-2"
          >
            Eliminate Cluster
          </button>
        </div>
        ...
        >
          <Box className="d-flex flex-column align-items-center" sx={style}>
            ...
            >
              <InfoIcon
                sx={{ color: "#1976D2", margin: "10px 2px", fontSize: "28px" }}
              />
              You can still retrieve the cluster from the deleted group.
            </p>
            <div className="d-flex justify-content-evenly">
              <button disabled onClick={handleDelete} className="mt-2 btn-1">
                Yes
              </button>
              <button
                onClick={() => setOpenModal(false)}
                className="mt-2 ms-2 btn-2"
              >
                No
              </button>
            </div>
          ...
          <Box className="d-flex flex-column align-items-center" sx={style}>
            ...
            <div className="email-draft">
              ...
              <textarea
                ...
                onChange={(event) =>
                  setDraftChange({ ...draftChange, body: event.target.value })
                }
                id="w3review"
                name="w3review"
                rows="4"
            ...
            <div className="d-flex justify-content-evenly align-items-center">
              <button onClick={handleDraftChange} className="mt-2 btn-2 mx-1">
                Change
              </button>
              <button
                onClick={() => setOpenEmailModal(false)}
                className="edit-button2 mt-2 mx-1"
              >
                Keep as is
              </button>
            </div>
          ...
          <Box className="d-flex flex-column align-items-center" sx={style}>
            ...
            >
              <InfoIcon
                sx={{ color: "#1976D2", margin: "10px 2px", fontSize: "28px" }}
              />
              This package is generated on the basis of buying patterns.
            </p>
            ...
            <div className="package-container w-100">
              ...
              <div className="d-flex justify-content-center">
                <div className="package-ul">
                  {clusters?.map((product, index) => (
                    <p key={index}>
                      <img
                        src={productImage || product?.productInfo.productImage}
                        className="package-img"
                      />
                      {`Product ${index + 1}` ||
                        product.productInfo.productName}{" "}
                      x{" "}
                      {Math.round(index * Math.random() + 1) ||
                        product.quantity}
                    </p>
                  ))}
                </div>
            ...
            <div className="d-flex justify-content-evenly">
              <button onClick={handlePackageClose} className="mt-2 ms-2 btn-2">
                Close
              </button>
            </div>
          </Box>
